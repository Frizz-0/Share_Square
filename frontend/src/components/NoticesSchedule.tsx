...
import { ArrowLeft, Megaphone, CalendarClock, Send, Trash2, Plus } from 'lucide-react';
...
export default function NoticesSchedule({ groupId, userId, isAdmin, recurringExpenses, onBack }: Props) {
  ...
  const handleAddSchedule = async (e: React.FormEvent) => {
    ...
    if (!scheduleTitle.trim() || isNaN(amt) || amt <= 0) return;
    try {
      const created = await createSchedule(groupId, userId, scheduleTitle.trim(), amt, parseInt(scheduleDay) || 1);
  ...
  const upcoming = [
    ...recurringExpenses.map(exp => ({ key: `exp-${exp.id}`, title: exp.title, amount: exp.convertedAmountGBP, dueDay: exp.recurringDay ?? 1, deletable: false as const, expenseId: null as string | null })),
    ...schedules.map(s => ({ key: `sch-${s.expenseId}`, title: s.title, amount: s.amount, dueDay: s.dueDay, deletable: true as const, expenseId: s.expenseId }))
    ...
    <div className="flex flex-col h-full min-h-[88vh]">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-black tracking-wider text-stone-400 uppercase flex items-center gap-1.5">
              <CalendarClock size={14} /> Upcoming Schedules
            </h3>
            <button onClick={() => setIsAddingSchedule(v => !v)} className="text-[10px] font-black text-emerald-700 flex items-center gap-1">
              <Plus size={12} /> Add
            </button>
          </div>

          {isAddingSchedule && (
            <form onSubmit={handleAddSchedule} className="p-4 bg-stone-50 rounded-2xl border border-stone-100 space-y-2.5 mb-3">
              <input type="text" placeholder="e.g., Council Tax" value={scheduleTitle} onChange={(e) => setScheduleTitle(e.target.value)} className="w-full bg-white border rounded-xl px-3 py-2.5 text-sm font-semibold focus:outline-none" required autoFocus />
              <div className="flex gap-2">
                <input type="number" step="0.01" placeholder="Amount (£)" value={scheduleAmount} onChange={(e) => setScheduleAmount(e.target.value)} className="flex-1 bg-white border rounded-xl px-3 py-2.5 text-sm font-bold focus:outline-none" required />
                <input type="number" min="1" max="31" placeholder="Day" value={scheduleDay} onChange={(e) => setScheduleDay(e.target.value)} className="w-20 bg-white border rounded-xl px-3 py-2.5 text-sm font-bold text-center focus:outline-none" required />
              </div>
          ...
          {isLoadingSchedules ? (
            ...
            <div className="p-4 text-center bg-stone-50/50 border border-dashed border-stone-200 rounded-2xl text-sm text-stone-400 font-medium">No upcoming schedules yet.</div>
          ) : (
            <div className="space-y-2.5">
              {upcoming.map(item => (
                <div key={item.key} className="p-4 bg-white border border-stone-200 rounded-2xl flex items-center justify-between text-sm shadow-sm">
                  <div className="flex items-center gap-2 text-stone-700 font-bold">
                    <span className="text-[10px] font-black text-emerald-700 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100">Day {item.dueDay}</span>
                    <span>{item.title}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="font-black text-stone-800 text-base">£{item.amount.toFixed(2)}</span>
                    {item.deletable && item.expenseId && (
                      <button onClick={() => setPendingDeleteSchedule(item.expenseId)} className="text-stone-300 hover:text-rose-600 transition-colors">
                        <Trash2 size={13} />
                      </button>
                    )}
                  </div>
      ...
      {pendingDeleteSchedule && (
        <ConfirmDialog
          ...
          message="This will remove the upcoming schedule entry for everyone in the group."
